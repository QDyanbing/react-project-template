...
- 基于 Fetch 标准，继续使用原生的请求、响应、Headers 和 AbortSignal 语义。
...
业务接口统一由 Service 层定义，页面和组件不直接处理底层请求。原生 Fetch 可以减少依赖和额外 API，同时避免模板默认加入拦截器、自动重试等可能改变请求行为的机制。

Fetch 不会因 4xx 或 5xx 响应自动拒绝 `Promise`，JSON 解析、请求超时和公共错误处理需要根据接口约定显式完成。这些能力直接基于标准 API 实现，不再引入 Axios、Ky 或 ofetch。
